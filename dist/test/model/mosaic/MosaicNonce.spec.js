...
describe('MosaicNonce', () => {
    it('should be created from Uint8Array', () => {
        const nonce = new MosaicNonce_1.MosaicNonce(new Uint8Array([0x12, 0x34, 0x56, 0x78]));
        assert_1.deepEqual(nonce.nonce, new Uint8Array([0x12, 0x34, 0x56, 0x78]));
        assert_1.deepEqual(nonce.toDTO(), new Uint8Array([0x12, 0x34, 0x56, 0x78]));
    });
    ...
    it('should create nonce from hexadecimal notation', () => {
        const nonce = MosaicNonce_1.MosaicNonce.createFromHex('12345678');
        chai_1.expect(nonce.nonce).to.not.be.null;
        assert_1.deepEqual(nonce.nonce, new Uint8Array([0x12, 0x34, 0x56, 0x78]));
    });
});
