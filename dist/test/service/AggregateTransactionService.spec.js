...
describe('AggregateTransactionService', () => {
    ...
    it('Should add cosignatories to signed Aggregate complete transaction', () => {
        ...
        chai_1.expect(() => {
            AggregateTransactionService_1.AggregateTransactionService.addCosignatures(AggregateTransaction_1.AggregateTransaction.createBonded(Deadline_1.Deadline.create(), [
            ...
            ], NetworkType_1.NetworkType.MIJIN_TEST, []).signWith(accountAlice, generationHash), []);
        ...
        const signedTxBob = model_1.CosignatureTransaction.signTransactionPayload(accountBob, aggregateTransactionPayload, generationHash);
        // 02.2 Carol cosigns the tx and sends it back to Alice
        const signedTxCarol = model_1.CosignatureTransaction.signTransactionPayload(accountCarol, aggregateTransactionPayload, generationHash);
        ...
        const cosignatureSignedTransactions = [
            new model_1.CosignatureSignedTransaction(signedTxBob.parentHash, signedTxBob.signature, signedTxBob.signer),
            new model_1.CosignatureSignedTransaction(signedTxCarol.parentHash, signedTxCarol.signature, signedTxCarol.signer),
        ];
        const recreatedTx = utility_1.TransactionMapping.createFromPayload(aggregateTransactionPayload);
        ...
        chai_1.expect(signedTransaction.type).to.be.equal(model_1.TransactionType.AGGREGATE_COMPLETE);
        ...
        const signedByAll = AggregateTransactionService_1.AggregateTransactionService.addCosignatures(signedOnlyByAlice, cosignatureSignedTransactions);
        chai_1.expect(signedByAll.hash).to.be.equal(signedTransaction.hash);
        chai_1.expect(signedByAll.networkType).to.be.equal(signedTransaction.networkType);
        chai_1.expect(signedByAll.payload).to.be.equal(signedTransaction.payload);
        chai_1.expect(signedByAll.signer).to.be.equal(signedTransaction.signer);
        chai_1.expect(signedByAll.type).to.be.equal(signedTransaction.type);
        const signedByAllAgain = AggregateTransactionService_1.AggregateTransactionService.addCosignatures(signedByAll, cosignatureSignedTransactions);
        chai_1.expect(signedByAllAgain.hash).to.be.equal(signedTransaction.hash);
        chai_1.expect(signedByAllAgain.networkType).to.be.equal(signedTransaction.networkType);
        chai_1.expect(signedByAllAgain.payload).to.be.equal(signedTransaction.payload);
        chai_1.expect(signedByAllAgain.signer).to.be.equal(signedTransaction.signer);
        chai_1.expect(signedByAllAgain.type).to.be.equal(signedTransaction.type);
    });
